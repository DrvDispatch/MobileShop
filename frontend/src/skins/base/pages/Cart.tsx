 ...
 * Base Skin - Cart Page
 * 
 * Purely presentational. Receives data via props from CartPageVM.
...
export function CartPage({ vm }: { vm: CartPageVM }) {
    const { items, subtotal, mounted, isEmpty, updateQuantity, removeItem, clearCart } = vm;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {items.map((item) => (
                        <div key={item.id} className="flex gap-4 p-4 bg-zinc-50 rounded-xl">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-lg font-semibold text-zinc-900 mt-1">€{item.price.toFixed(2)}</p>

                                {/* Quantity */}
                                <div className="flex items-center gap-3 mt-3">
                                    <div className="flex items-center border border-zinc-200 rounded-lg bg-white">
                                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-2 hover:bg-zinc-100 transition-colors">
                    ...
                    <button onClick={clearCart} className="text-sm text-zinc-500 hover:text-red-500 transition-colors">
                        Clear cart
                    </button>
                </div>
