...
/**
 * Base Skin - Account Dashboard Page
 * 
 * PURE PRESENTATION ONLY
 * - Receives AccountPageVM via props
...
import type { AccountPageVM, AccountOrderVM } from '@core/hooks/pages/useAccountPageVM';
import {
    Package, Lock, Settings, LogOut, ChevronRight, Truck,
    ChevronDown, ChevronUp, ShoppingBag, Calendar,
    Star, Gift, Heart, Shield, FileDown
} from 'lucide-react';
...
function OrderCard({ order, onDownloadInvoice }: {
    order: AccountOrderVM;
    onDownloadInvoice: (e: React.MouseEvent, orderId: string, orderNumber: string) => void;
}) {
    const getBorderClass = () => {
        switch (order.status) {
            case "PENDING": return "border-l-yellow-500 bg-yellow-50/30";
            case "PAID":
            case "PROCESSING": return "border-l-blue-500";
            case "SHIPPED": return "border-l-purple-500";
            case "DELIVERED": return "border-l-green-500";
            case "REFUNDED": return "border-l-orange-500 bg-orange-50/30";
            case "CANCELLED": return "border-l-red-500 bg-red-50/30 opacity-75";
            default: return "border-l-zinc-300";
        }
    };

    const getPriceClass = () => {
        if (order.status === "REFUNDED") return "text-orange-600 line-through";
        if (order.status === "CANCELLED") return "text-red-400 line-through";
    ...
    return (
        <Link
            ...
            className={`block p-5 hover:bg-zinc-50 transition-colors border-l-4 ${getBorderClass()}`}
        >
            {/* Order Header */}
            ...
            <div className="flex items-center gap-3">
                <div className="flex -space-x-3">
                    {order.items.slice(0, 4).map((item, i) => (
                        <div
                            key={i}
                            className="w-14 h-14 rounded-lg border-2 border-white bg-zinc-100 overflow-hidden flex-shrink-0 shadow-sm flex items-center justify-center"
                        >
                            {item.productImage ? (
                                <img
                ...
                <div className="flex-1 min-w-0 ml-2">
                    <p className="text-sm text-zinc-600 truncate">{order.itemSummary}</p>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                        onClick={(e) => onDownloadInvoice(e, order.id, order.orderNumber)}
...
export function AccountPage({ vm }: AccountPageProps) {
    // Loading state
    if (vm.isLoading) {
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
                ...
                <p className="text-zinc-500 mt-1">
                    Welkom terug, {vm.user?.firstName}! Bekijk je bestellingen en beheer je account.
                </p>
            </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-zinc-100">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div className="flex-1 min-w-0">
                                <h2 className="text-xl font-semibold text-zinc-900 truncate">{vm.user?.name}</h2>
                                <p className="text-zinc-500 text-sm truncate">{vm.user?.email}</p>
                                {vm.user && !vm.user.emailVerified && (
                        ...
                        <div className="space-y-2">
                            ...
                            <button onClick={vm.onLogout} className="flex items-center gap-3 p-3 rounded-xl hover:bg-red-50 transition-colors group w-full">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-zinc-100 overflow-hidden">
                        ...
                        <div className="p-6 border-b border-zinc-100 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="bg-zinc-100 text-zinc-600 text-sm px-2.5 py-0.5 rounded-full">
                                    {vm.totalOrders}
                                </span>
                            </div>
                            {vm.hasOrders && (
                            ...
                            <div className="p-4 border-t border-zinc-100">
                                ...
                                >
                                    {vm.showAllOrders ? (
                                        ...
                                        <>Toon alle {vm.totalOrders} bestellingen <ChevronDown className="w-4 h-4" /></>
                                    )}
                                </button>
                            </div>
                        )}
