...
/**
 * Base Skin - Checkout Page
 * 
 * Purely presentational. Receives all data via props from CheckoutPageVM.
 */
...
export function CheckoutPage({ vm }: { vm: CheckoutPageVM }) {
    const {
        mounted, items, isEmpty, formData, phonePrefix, isLoggedIn,
        couponCode, appliedDiscount, couponError, couponLoading,
        subtotal, shipping, discount, total, isLoading, error, countryCodes, labels,
        setFormData, setPhonePrefix, setCouponCode, validateCouponCode, removeCoupon, handleSubmit
    } = vm;
                            ...
                            <div className="bg-white rounded-xl p-6 shadow-sm">
                                ...
                                <div className="grid sm:grid-cols-2 gap-4">
                                    <div className="sm:col-span-2">
                                        ...
                                        />
                                        {isLoggedIn && (
                                            <p className="text-xs text-zinc-500 mt-1">{labels.confirmationNote}</p>
                                        )}
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {countryCodes.map((c) => (
                                                    <option key={c.code} value={c.prefix}>{c.prefix}</option>
                                                ))}
                                            </select>
                                            <Input
                                                id="phone"
                            ...
                            <div className="bg-white rounded-xl p-6 shadow-sm sticky top-24">
                                ...
                                <div className="border-t border-zinc-200 pt-4 mb-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-sm font-medium text-zinc-700">{labels.couponCode}</span>
                                    </div>
                                    {appliedDiscount ? (
                                        <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                                            ...
                                            <button type="button" onClick={removeCoupon} className="text-zinc-400 hover:text-red-500">
                                                <X className="w-4 h-4" />
                                            </button>
                                        </div>
                                    ) : (
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {couponLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : labels.apply}
                                            </Button>
                                        </div>
                                    )}
                                    {couponError && (
                                ...
                                <div className="border-t border-zinc-200 pt-4 space-y-2">
                                    ...
                                    {discount > 0 && (
                                        <div className="flex justify-between text-sm">
                                            <span className="text-green-600">{labels.discount}</span>
