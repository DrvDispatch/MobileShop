...
/**
 * Base Skin - Phones Page
 * 
 * Purely presentational. Receives data via props from PhonesPageVM.
 */
...
export function PhonesPage({ vm }: { vm: PhonesPageVM }) {
    const {
        products, allProducts, isLoading, currentPage, totalPages, goToPage, getPageNumbers,
        ...
        BRANDS, CONDITIONS, STORAGE_OPTIONS, BATTERY_OPTIONS, SORT_OPTIONS,
    } = vm;
                ...
                <div className="flex-1">
                    ...
                    {!isLoading && totalPages > 1 && (
                        <div className="flex items-center justify-center gap-2 mt-8 pb-8">
                            <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className={`flex items-center gap-1 px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${currentPage === 1 ? 'border-zinc-200 text-zinc-300 cursor-not-allowed' : 'border-zinc-300 text-zinc-700 hover:bg-zinc-50'}`}>
                            ...
                            <div className="flex items-center gap-1">
                                {getPageNumbers().map((page, idx) => page === '...' ? (
                                    ...
                                    <button key={page} onClick={() => goToPage(page as number)} className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${currentPage === page ? 'bg-zinc-900 text-white' : 'text-zinc-700 hover:bg-zinc-100'}`}>{page}</button>
                                ))}
                            </div>
                            <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className={`flex items-center gap-1 px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${currentPage === totalPages ? 'border-zinc-200 text-zinc-300 cursor-not-allowed' : 'border-zinc-300 text-zinc-700 hover:bg-zinc-50'}`}>
