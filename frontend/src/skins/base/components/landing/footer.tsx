...
import type { FooterVM } from '@core/skin';

interface FooterProps {
    vm: FooterVM;
}

export function Footer({ vm }: FooterProps) {
    const {
        shopName,
        contactInfo,
        shopLinks,
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="text-center md:text-left">
                            ...
                            <p className="text-zinc-400 text-sm">{newsletterSubtitle}</p>
                        </div>
                        <form onSubmit={onSubscribe} className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className={`grid grid-cols-2 ${shopLinks.length > 0 ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-8`}>
                    ...
                    <div className="col-span-2 md:col-span-1">
                        ...
                        <p className="text-sm text-zinc-500 mb-4">{tagline}</p>
                    ...
                    {shopLinks.length > 0 && (
                        <div>
                            ...
                            <ul className="space-y-2 text-sm text-zinc-500">
                                {shopLinks.map(link => (
                                    <li key={link.href}>
                                        <Link href={link.href} className="hover:text-zinc-900 transition-colors">{link.label}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div>
                        <h3 className="text-sm font-medium text-zinc-900 mb-4">{labels.services}</h3>
                        <ul className="space-y-2 text-sm text-zinc-500">
                    ...
                    {/* Juridisch */}
                    <div>
                        <h3 className="text-sm font-medium text-zinc-900 mb-4">{labels.legal}</h3>
                        <ul className="space-y-2 text-sm text-zinc-500">
                            <li><Link href="/privacy" className="hover:text-zinc-900 transition-colors">{labels.privacy}</Link></li>
                            <li><Link href="/terms" className="hover:text-zinc-900 transition-colors">{labels.terms}</Link></li>
                            <li><Link href="/returns" className="hover:text-zinc-900 transition-colors">{labels.returns}</Link></li>
                        </ul>
                            ...
                            >
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-xl font-bold text-zinc-900">{googleReviewRating || '4.7'}</span>
                                        <div className="flex">
                                            {[1, 2, 3, 4].map((i) => (
                ...
                <div className="mt-12 pt-8 border-t border-zinc-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-4 text-sm text-zinc-400">
                        ...
                        <span>•</span>
                        <span>Prijzen {taxIncluded ? 'incl.' : 'excl.'} {taxLabel}</span>
                    </div>
                </div>
            </div>
        </footer>
