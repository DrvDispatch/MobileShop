...
export function Footer() {
    ...
    // Tenant and UI config
    const tenant = useTenant();
    const { uiConfig } = useUIConfig();
    const { footer: footerConfig, labels } = uiConfig;
    ...
    // Uses labels from UIConfig
    const shopLinks = ecommerceEnabled ? [
        { href: "/phones", label: labels.nav.devices },
    ...
    const dienstenLinks = [
        ...
        { href: "/over-ons", label: labels.nav.about },
    ];

    // Get contact info from tenant config, with fallback to settings
    const contactInfo = {
        address: tenant.contact.address as { line1?: string; postalCode?: string; city?: string } | null,
        phone: tenant.contact.phone || store.phone,
        email: tenant.contact.email || store.email,
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="text-center md:text-left">
                            <h3 className="text-xl font-semibold mb-2">{footerConfig.newsletterTitle}</h3>
                        ...
                        <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                            <div className="relative">
                                <input
                                    ...
                                    placeholder={labels.footer.emailPlaceholder}
                                    required
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className={`grid grid-cols-2 ${shopLinks.length > 0 ? 'md:grid-cols-4' : 'md:grid-cols-3'} gap-8`}>
                    ...
                    <div className="col-span-2 md:col-span-1">
                        <Link href="/" className="flex items-center gap-2 mb-4">
                            ...
                            <span className="font-semibold text-zinc-900">{tenant.branding.shopName}</span>
                        </Link>
                        ...
                        <div className="space-y-2 text-sm text-zinc-500 mb-4">
                            {contactInfo.address && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>{contactInfo.address.line1}, {contactInfo.address.postalCode} {contactInfo.address.city}</span>
                                </div>
                            )}
                            {contactInfo.phone && (
                                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-medium text-zinc-900 mb-4">{labels.footer.services}</h3>
                        <ul className="space-y-2 text-sm text-zinc-500">
                    ...
                    {/* Juridisch */}
                    <div>
                        <h3 className="text-sm font-medium text-zinc-900 mb-4">{labels.footer.legal}</h3>
                        <ul className="space-y-2 text-sm text-zinc-500">
                            <li><Link href="/privacy" className="hover:text-zinc-900 transition-colors">{labels.footer.privacy}</Link></li>
                            <li><Link href="/terms" className="hover:text-zinc-900 transition-colors">{labels.footer.terms}</Link></li>
                            <li><Link href="/returns" className="hover:text-zinc-900 transition-colors">{labels.footer.returns}</Link></li>
                        </ul>

                        {/* Google Reviews Badge - only show if URL is configured */}
                        {footerConfig.googleReviewUrl && (
                            <a
                                href={footerConfig.googleReviewUrl}
                                target="_blank"
                            ...
                            >
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-xl font-bold text-zinc-900">{footerConfig.googleReviewRating || '4.7'}</span>
                                        <div className="flex">
                                            {[1, 2, 3, 4].map((i) => (
                ...
                <div className="mt-12 pt-8 border-t border-zinc-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    <p className="text-sm text-zinc-400">
                        © {new Date().getFullYear()} {tenant.branding.shopName}. Alle rechten voorbehouden.
                    </p>
